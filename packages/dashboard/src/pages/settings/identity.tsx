...
export default function Index() {
	...
	const create = async (data: EmailValues) => {
		toast.promise(
			...
			}),
			{
				...
				success: (res) => {
					void identityMutate({ provider: res.provider, tokens: res.tokens, records: res.records }, { revalidate: false });
					void projectsMutate();
	...
	const isSendGrid = identity.provider === "sendgrid";
					...
					{activeProject.email && !activeProject.verified ? (
						<>
							<Alert type={"warning"} title={"Waiting for DNS verification"}>
								Please add the following {isSendGrid ? "SendGrid" : "SES"} DNS records to {activeProject.email.split("@")[1]} to
								verify {activeProject.email}, this may take up to 15 minutes to register. <br />
							...
							<div className="mt-6">
								<Table
									values={records.map((record) => ({
										Type: <Badge type={"info"}>{record.type}</Badge>,
										Host: (
											<div
												className={"flex cursor-pointer items-center gap-3"}
												onClick={() => {
													void navigator.clipboard.writeText(record.host);
													toast.success("Copied host to clipboard");
												}}
											>
												<p className={"font-mono text-sm"}>{record.host}</p>
												<Copy size={14} />
											</div>
										),
										Value: (
