...
<Callout title="Postal self-hosting" type="info">
  When `EMAIL_PROVIDER=postal`, Plunk creates and refreshes domains through Postal's legacy domain API. Configure
  `POSTAL_DOMAIN_API_BASE_URL` and add the DNS records returned by Postal in the dashboard.
</Callout>
