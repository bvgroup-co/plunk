...
Plunk uses AWS SES by default (`EMAIL_PROVIDER=ses`). If you want to use SendGrid for outbound sending instead, set `EMAIL_PROVIDER=sendgrid` and provide `SENDGRID_API_KEY`. To send directly through Postal, set `EMAIL_PROVIDER=postal` and provide the Postal settings below. SES inbound email and SNS/SQS event ingestion remain SES-only features.
...
For Postal mode, configure direct sending and direct Postal webhook ingestion:
...
POSTAL_DOMAIN_API_KEY="postal-domain-management-key"
# Optional: target a specific Postal server when the domain-management service needs it
# POSTAL_DOMAIN_SERVER_ID="server-id"
# POSTAL_DOMAIN_SERVER_PERMALINK="server-permalink"
POSTAL_WEBHOOK_SECRET="shared-secret"
```

Configure Postal to POST message events directly to `/webhooks/postal/events`. If `POSTAL_WEBHOOK_SECRET` is set, include it in the `X-Plunk-Postal-Webhook-Secret` header or as a `secret` query parameter. Postal domain setup calls the configured Postal domain-management API, stores the returned Postal domain ID, and displays the DNS records returned by Postal. Those records may include SPF, DKIM, return-path, MX, tracking, and ownership-verification TXT records. Plunk does not generate a synthetic Postal CNAME record.
