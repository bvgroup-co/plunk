...
Plunk uses AWS SES by default (`EMAIL_PROVIDER=ses`). If you want to use SendGrid for outbound sending instead, set `EMAIL_PROVIDER=sendgrid`, provide `SENDGRID_API_KEY`, and skip the SES outbound setup below. SES domain verification, inbound email, and SNS/SQS event ingestion remain SES-only features.
