...
# Optional: defaults to POSTAL_API_KEY when unset
POSTAL_DOMAIN_API_KEY="postal-domain-api-key"
POSTAL_WEBHOOK_SECRET="shared-secret"
```

Configure Postal to POST message events directly to `/webhooks/postal/events`. If `POSTAL_WEBHOOK_SECRET` is set, include it in the `X-Plunk-Postal-Webhook-Secret` header or as a `secret` query parameter.

In Postal mode, Plunk calls Postal's legacy domain API to create, refresh, and delete domains:

- `POST /api/v1/domains`
- `POST /api/v1/domains/:id/check`
- `DELETE /api/v1/domains/:id`

Postal is the source of truth for DNS records and verification status. The dashboard stores and displays the DNS records Postal returns instead of generating provider-specific records in Plunk.
