...
Configure Postal to POST message events directly to `/webhooks/postal/events`. If `POSTAL_WEBHOOK_SECRET` is set, include it in the `X-Plunk-Postal-Webhook-Secret` header or as a `secret` query parameter. Postal domain setup displays a Plunk-owned CNAME record (`mail.your-domain` by default) pointing to `POSTAL_CNAME_VALUE`; when `POSTAL_DNS_CHECK_ENABLED=true`, Plunk verifies that CNAME directly.
