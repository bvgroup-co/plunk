...
Plunk sends through AWS SES by default. Set `EMAIL_PROVIDER=sendgrid` to send outbound email with SendGrid, or `EMAIL_PROVIDER=postal` to send through Postal. Postal mode requires Postal's legacy send API and domain API endpoints so Plunk can provision, verify, and delete sender domains. SES inbound email and SES SNS/SQS event ingestion remain SES-only features.
...
| `POSTAL_DOMAIN_API_BASE_URL`                 | Yes when `EMAIL_PROVIDER=postal` | Postal base URL exposing `/api/v1/domains` domain-management endpoints.                 | `https://postal.example.com` |
| `POSTAL_DOMAIN_API_KEY`                      | No                               | Optional Postal API key for domain management. Defaults to `POSTAL_API_KEY`.            | `postal-domain-key`          |
| `POSTAL_DOMAIN_SERVER_ID`                    | No                               | Optional server ID included in Postal domain create requests.                           | `server-123`                 |
| `POSTAL_DOMAIN_SERVER_PERMALINK`             | No                               | Optional server permalink included when a domain API requires explicit server selection. | `mail-server`                |
...
Postal mode provisions sender domains through Postal's legacy domain API and displays the DNS records returned by Postal in the dashboard.
