...
| `POSTAL_CNAME_VALUE`                         | Yes when `EMAIL_PROVIDER=postal` | DNS CNAME target displayed for Postal sender domains.                                                            | `postal.example.com`         |
| `POSTAL_DOMAIN_AUTH_SUBDOMAIN`               | No                               | Subdomain used for the displayed Postal CNAME record.                                                            | `mail` (default)             |
| `POSTAL_DNS_CHECK_ENABLED`                   | No                               | When true, Plunk resolves the Postal CNAME and marks matching domains verified. When false, domains stay manual. | `false` (default)            |
