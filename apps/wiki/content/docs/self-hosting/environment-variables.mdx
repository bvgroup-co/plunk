...
Plunk sends through AWS SES by default. Set `EMAIL_PROVIDER=sendgrid` to send outbound email with SendGrid, or `EMAIL_PROVIDER=postal` to send through Postal. Stock Postal is send/webhook only in Plunk: it does not provide a supported domain-management API, so sender domains must be created and verified in Postal outside Plunk. SES inbound email and SES SNS/SQS event ingestion remain SES-only features.
...
Postal mode does not provision sender domains in stock Postal. Add the domain and copy DNS records from Postal's admin UI before sending from that domain in Plunk.
