...
| `SES_EVENTS_SQS_ENABLED`                    | No                    | Enables worker pull-mode ingestion for SES events delivered by SNS to SQS. Keep `false` to use only the existing `POST /webhooks/sns` endpoint.                                                                                                                                                                                                                      | `false`                                         |
| `SES_EVENTS_SQS_QUEUE_URL`                  | Required when enabled | SQS queue URL subscribed to the SES SNS topic.                                                                                                                                                                                                                                                                                                                       | `https://sqs.us-east-1.amazonaws.com/123/queue` |
| `SES_EVENTS_SQS_REGION`                     | No                    | AWS region for the SQS queue. Defaults to `AWS_SES_REGION`.                                                                                                                                                                                                                                                                                                          | `us-east-1`                                     |
| `SES_EVENTS_SQS_ACCESS_KEY_ID`              | No                    | AWS access key ID for SQS polling. Defaults to `AWS_SES_ACCESS_KEY_ID`.                                                                                                                                                                                                                                                                                              | `AKIA...`                                       |
| `SES_EVENTS_SQS_SECRET_ACCESS_KEY`          | No                    | AWS secret access key for SQS polling. Defaults to `AWS_SES_SECRET_ACCESS_KEY`.                                                                                                                                                                                                                                                                                      | `wJalr...`                                      |
| `SES_EVENTS_SQS_WAIT_TIME_SECONDS`          | No                    | SQS long-poll wait time.                                                                                                                                                                                                                                                                                                                                             | `20`                                            |
| `SES_EVENTS_SQS_MAX_MESSAGES`               | No                    | Maximum SQS messages fetched per poll.                                                                                                                                                                                                                                                                                                                               | `10`                                            |
| `SES_EVENTS_SQS_VISIBILITY_TIMEOUT_SECONDS` | No                    | SQS visibility timeout used while processing a batch.                                                                                                                                                                                                                                                                                                                | `60`                                            |
| `SES_EVENTS_SQS_IDLE_DELAY_MS`              | No                    | Delay after an empty poll before polling again.                                                                                                                                                                                                                                                                                                                      | `1000`                                          |
| `SES_EVENTS_SQS_RAW_MESSAGE_DELIVERY`       | No                    | Set to `true` only when the SNS subscription uses raw message delivery. The SQS MessageId is then used for idempotency.                                                                                                                                                                                                                                              | `false`                                         |
